...
function remove_element_atIndex(arr, index) {
 ...
 var validPath = function (n, edges, source, destination) {
    ...
    for (var i = 0; i < edges.length; i++) {
      for (var j = 0; j < edges[i].length; j++) {
        isOut = false;
        if (edges[i][j] === source) {
          let newSource = edges[i][j === 0 ? 1 : 0];
          let newEdges = remove_element_atIndex(edges, i);
          isFound = validPath(n, newEdges, newSource, destination);
          break;
        }
      }
